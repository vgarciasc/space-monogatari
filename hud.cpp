...
void iniciar_salvar_score (Hud *hud){
	while (!al_is_event_queue_empty(hud->event_queue)) {
		ALLEGRO_EVENT evento;
		al_wait_for_event(hud->event_queue, &evento);
...
void ler_high_score (Hud *hud) {
	...
	for (int i = 0; i < TOP_PLAYERS; i++)
		fscanf(hud->arquivo_high_score,
			" %c %c %c %d\n",
			&hud->nomes_high_score[i][0],
			&hud->nomes_high_score[i][1],
			&hud->nomes_high_score[i][2],
...
void inicializar_high_score(Hud *hud) {
    hud->event_queue = al_create_event_queue();
    al_register_event_source(hud->event_queue, al_get_keyboard_event_source());
}
...
void enviar_score(Hud *hud) {
	...
	if (hud->nome_score[0] != '_') {
		for (int i = 0; i < 3; i++)
			hud->nomes_high_score[TOP_PLAYERS - 1][i] = hud->nome_score[i];

		hud->scores_high_score[TOP_PLAYERS - 1] = hud->score;
		ordenar_vetor(hud->scores_high_score, hud->nomes_high_score, TOP_PLAYERS);
